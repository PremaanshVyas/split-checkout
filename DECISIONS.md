...
## 2026-07-03: Spending mandates, the delegation layer agents actually need

**Decision:** Agents no longer need to name cards. A human creates a **mandate** in the store's Agent mode (budget, expiry, backing cards) and hands the agent a short code; `split_purchase` accepts the code and the server enforces the rest. Enforcement rules, each deliberate: over-budget spends are refused **before any payment intent is created**; the budget decrements only when an order **captures** (a declined card costs nothing, mirroring the capture-together gate's own semantics); expiry and revocation are re-checked on every spend; **refunds do not restore budget** (a mandate is a spend cap the human granted, not a wallet balance; letting refunds top it back up would let an agent cycle purchases to exceed the human's intended exposure).

**The design maps onto the field on purpose.** ACP's `allowance` object (max_amount, expires_at, one-time scope), AP2's user-signed Intent mandates, and Visa/Mastercard's agentic tokens all express the same idea: the agent holds a scoped grant, never a credential. What none of them express is a grant spanning **multiple funding sources with the budget enforced across all of them together**, which is precisely what a split-payment mandate is. Airwallex's Airi roadmap ("delegated agent payments with spend limits and permission controls") describes this layer; the demo implements a working version of it on Airwallex rails.

**Demo trade-offs, named honestly:** mandate codes are bearer tokens with no agent authentication (production wants the second layer the standards define: agent identity via signed requests); `create_demo_mandate` exists so the MCP flow can be tried without visiting the store UI, with its description stating plainly that in production this is a wallet action the human authorizes; budget accounting is single-process (no lock), consistent with the demo's existing concurrency posture. Covered by six unit tests including the decline-costs-nothing and refund-does-not-restore cases, and verified end to end: a $600 mandate refused a $1,950 bundle, bought a $485 grinder split across its two cards, then refused a $189 kettle against the $115 remainder.

---

## 2026-07-03: Self-describing split parts, and the operator's case (OPERATORS.md)

**Decision:** Every split part is now legible as one order from every seat at the table. Each PaymentIntent carries a part-labelled descriptor ("AURORA 1/2") for the shopper's bank statement, and metadata that reconciles for the merchant and explains itself to anyone inside Airwallex: shared `order_ref`, `split_part`, `order_total_aud`, and a plain-language `note` ("Split-tender: part 1 of 2 of one 485.00 AUD order. Siblings share order_ref..."). Verified round-tripping through the live sandbox; Airwallex's settlement report exposes order id and metadata as columns, so the two rows group into one order with zero new platform machinery. Sandbox caveat recorded honestly: descriptor display is account-controlled (the sandbox shows the account name), so the field is populated correctly but its statement rendering is a production account setting.

**Addendum, 2026-07-03, the card-type pass:** dedicated research into what actually flows through Airwallex checkouts reshaped the analysis and the demo. Australia's card mix is 76.6% debit by transaction count with mobile wallets carrying 43% of card transactions (RBA data), which strengthens the rescue case since debit declines are balance-driven. A wallet can be one leg of a split with no documented blocker, and it is the better leg twice over: tokenised online debit interchange is 0.10% versus 0.20%, and cryptogram-authenticated wallet payments skip the 3DS challenge. Amex rides at Airwallex's same list price (unusual among PSPs), sits outside the RBA designation so it stays surchargeable after October 2026, and splits fine: a Visa leg plus an Amex leg captured together is verified in EVIDENCE.md. The demo's aliases now cover all seven schemes Airwallex accepts. Two caveats recorded: Airwallex does not expose partial authorization (so prepaid splits are amount-known-in-advance today), and JCB cannot complete 3DS per their docs history. OPERATORS.md gained the per-card-type table and a phased platform proposal (toggle, order object, Airi capability, network features) where no phase touches custody, licensing, or scheme filings.

**The companion research, distilled into [OPERATORS.md](OPERATORS.md):** the marginal cost of the second transaction on Airwallex AU pricing is one fixed fee, A$0.30 on a A$1,200 order, with interchange and scheme fees split-neutral for online cards; the RBA's 31 March 2026 Conclusions Paper bans surcharging (so the merchant absorbs it) while cutting credit interchange by roughly eight times that amount on the same order; Visa's rulebook permits two-cards-one-order by name (rule 0008603's exception list) and the prohibited "split sale" is the same-card pattern; Adyen's `/orders` partial-payments API is the blueprint for a platform-native order object, which is exactly the shape of this repo's `order_groups` table; and the costs that genuinely double (risk screening, worst-case dispute fees, monitoring event counts) are quantified and bounded. The adoption objection is answered without invented numbers: no public split-tender usage share exists, so the sizing case rests on decline rescue, where the feature costs nothing until the moment a sale is already failing.
