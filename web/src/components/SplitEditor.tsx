import { useMemo, useState } from "react";

interface Props {
  total: number;
  currency: string;
  busy: boolean;
  onConfirm: (splits: number[]) => void;
}

const fmt = (n: number, currency: string) =>
  new Intl.NumberFormat("en-AU", { style: "currency", currency }).format(n);

/**
 * Two amounts that must sum exactly to the cart total. The server
 * re-validates; this is purely for a friendly editing experience.
 */
export function SplitEditor({ total, currency, busy, onConfirm }: Props) {
  const [first, setFirst] = useState(Math.round((total / 2) * 100) / 100);

  const second = useMemo(() => Math.round((total - first) * 100) / 100, [first, total]);
  const valid = first >= 1 && second >= 1 && !Number.isNaN(first);

  const setClamped = (value: number) => {
    if (Number.isNaN(value)) {
      setFirst(NaN);
      return;
    }
    setFirst(Math.round(value * 100) / 100);
  };

  return (
    <section className="split-editor">
      <h2>Split your payment</h2>
      <p className="muted">
        Pay {fmt(total, currency)} across two cards. Neither card is charged until both
        authorizations succeed.
      </p>

      <input
        type="range"
        min={1}
        max={total - 1}
        step={0.01}
        value={Number.isNaN(first) ? total / 2 : first}
        onChange={(e) => setClamped(Number(e.target.value))}
        aria-label="Split amount for card 1"
      />

      <div className="split-inputs">
        <label>
          Card 1
          <input
            type="number"
            min={1}
            max={total - 1}
            step={0.01}
            value={Number.isNaN(first) ? "" : first}
            onChange={(e) => setClamped(e.target.value === "" ? NaN : Number(e.target.value))}
          />
        </label>
        <label>
          Card 2
          <input type="number" value={second} readOnly />
        </label>
      </div>

      <div className="split-presets">
        <button type="button" onClick={() => setClamped(total / 2)}>
          50 / 50
        </button>
        <button type="button" onClick={() => setClamped(Math.round(total * 75) / 100)}>
          75 / 25
        </button>
        {total > 161.02 && (
          <button
            type="button"
            title="Sets card 2 to $80.51, a sandbox amount that triggers an insufficient-funds decline with the matching test card"
            onClick={() => setClamped(Math.round((total - 80.51) * 100) / 100)}
          >
            Insufficient funds demo
          </button>
        )}
      </div>

      {!valid && <p className="error">Each card must carry at least 1.00.</p>}

      <button className="primary" disabled={!valid || busy} onClick={() => onConfirm([first, second])}>
        {busy ? "Setting up…" : "Continue to cards"}
      </button>
    </section>
  );
}
