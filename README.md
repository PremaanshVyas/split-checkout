...
- **Webhook-first status** — a signed webhook listener is implemented (HMAC-SHA256 verification, `payment_intent.*` events feeding the same state machine, safe under duplicate/out-of-order delivery), but the demo remains polling-primary so it runs deterministically from a fresh clone with no public URL. Production would invert that: webhooks drive, polling reconciles. To enable: register `https://<your-host>/api/webhooks/airwallex` in the Airwallex dashboard (Developer → Webhooks) and set `AIRWALLEX_WEBHOOK_SECRET`.
